...
this.makeHtml=function(text){if(g_urls)
...
function _DoItalicsAndBold(text){text=asciify(text);text=text.replace(/(^|[\W_])(?:(?!\1)|(?=^))(\*|_)\2(?=\S)([^\r]*?\S)\2\2(?!\2)(?=[\W_]|$)/g,"$1<strong>$3</strong>");text=text.replace(/(^|[\W_])(?:(?!\1)|(?=^))(\*|_)(?=\S)((?:(?!\2)[^\r])*?\S)\2(?!\2)(?=[\W_]|$)/g,"$1<em>$3</em>");return deasciify(text);}
...
function _DoAutoLinks(text){text=text.replace(autoLinkRegex,handleTrailingParens);var replacer=function(wholematch,m1){var url=encodeProblemUrlChars(m1);url=escapeCharacters(url,"*_");return"<a href=\""+url+"\">"+pluginHooks.plainLinkText(m1)+"</a>";};text=text.replace(/<((https?|ftp):[^'">\s]+)>/gi,replacer);return text;}
...
if(!needsRemoval)
return html;var ctag=0;html=html.replace(re,function(match){var res=tagremove[ctag]?"":match;ctag++;return res;});return html;}})();!function($){"use strict";var MarkdownEditor=function(element,options){this.$ns='bootstrap-markdown'
...
this.$nextTab=[]
this.showEditor()}
MarkdownEditor.prototype={constructor:MarkdownEditor,__alterButtons:function(name,alter){var handler=this.$handler,isAll=(name=='all'),that=this
...
if(typeof callbackContent=='string'){content=callbackContent}else{var val=this.$textarea.val();var converter=new Markdown.getSanitizingConverter();if(typeof markdown=='object'){content=markdown.toHTML(val);}else if(typeof marked=='function'){content=marked(val);}else if(typeof converter=='object'){content=converter.makeHtml(val);}else{content=val;}}
...
if(isHideable){if(editable.el!=null){var converter=new Markdown.getSanitizingConverter();var oldElement=$('<'+editable.type+'/>'),content=this.getContent(),currentContent=(typeof converter=='object')?converter.makeHtml(content):content
...
if(!data)$this.data('markdown',(data=new MarkdownEditor(this,options)))})}
...
e.preventDefault()}).on('click',function(e){analyzeMarkdown(e)}).on('focusin',function(e){analyzeMarkdown(e)}).ready(function(){$('textarea[data-provide="markdown"]').each(function(){initMarkdown($(this))})})}(window.jQuery);/*! jQuery UI - v1.10.4 - 2014-04-26
* http://jqueryui.com
...
next();});}};});})(jQuery);(function(factory){'use strict';if(typeof define==='function'&&define.amd){define(['jquery'],factory);}else{factory(window.jQuery);}}(function($){'use strict';var counter=0;$.ajaxTransport('iframe',function(options){if(options.async){var initialIframeSrc=options.initialIframeSrc||'javascript:false;',form,iframe,addParamChar;return{send:function(_,completeCallback){form=$('<form style="display:none;"></form>');form.attr('accept-charset',options.formAcceptCharset);addParamChar=/\?/.test(options.url)?'&':'?';if(options.type==='DELETE'){options.url=options.url+addParamChar+'_method=DELETE';options.type='POST';}else if(options.type==='PUT'){options.url=options.url+addParamChar+'_method=PUT';options.type='POST';}else if(options.type==='PATCH'){options.url=options.url+addParamChar+'_method=PATCH';options.type='POST';}
...
return this._getXHRPromise(false,data&&data.context);}});}));(function(e,t,n,r){var i="ontouchstart"in t,s=i?"touchstart":"mousedown",o=i?"touchmove":"mousemove",u=i?"touchend":"mouseup",a=i?"touchcancel":"mouseup",f=function(e){var t=e/3600,n=Math.floor(t),r=e%3600/60,i=Math.floor(r),s=Math.ceil(e%3600%60);if(s>59){s=0;i=Math.ceil(r)}if(i>59){i=0;n=Math.ceil(t)}return(n==0?"":n>0&&n.toString().length<2?"0"+n+":":n+":")+(i.toString().length<2?"0"+i:i)+":"+(s.toString().length<2?"0"+s:s)},l=function(e){var t=n.createElement("audio");return!!(t.canPlayType&&t.canPlayType("audio/"+e.split(".").pop().toLowerCase()+";").replace(/no/,""))};e.fn.audioPlayer=function(t){var t=e.extend({classPrefix:"audioplayer",strPlay:"Play",strPause:"Pause",strVolume:"Volume"},t),n={},r={playPause:"playpause",playing:"playing",stopped:"stopped",time:"time",timeCurrent:"time-current",timeDuration:"time-duration",bar:"bar",barLoaded:"bar-loaded",barPlayed:"bar-played",volume:"volume",volumeButton:"volume-button",volumeAdjust:"volume-adjust",noVolume:"novolume",muted:"muted",mini:"mini"};for(var u in r)n[u]=t.classPrefix+"-"+r[u];this.each(function(){if(e(this).prop("tagName").toLowerCase()!="audio")return false;var r=e(this),u=r.attr("src"),c=r.get(0).getAttribute("autoplay"),c=c===""||c==="autoplay"?true:false,h=r.get(0).getAttribute("loop"),h=h===""||h==="loop"?true:false,p=false;if(typeof u==="undefined"){r.find("source").each(function(){u=e(this).attr("src");if(typeof u!=="undefined"&&l(u)){p=true;return false}})}else if(l(u))p=true;var d=e('<div class="'+t.classPrefix+'">'+(p?e("<div>").append(r.eq(0).clone()).html():'<embed src="'+u+'" width="0" height="0" volume="100" autostart="'+c.toString()+'" loop="'+h.toString()+'" />')+'<div class="'+n.playPause+'" title="'+t.strPlay+'"><a href="#">'+t.strPlay+"</a></div></div>"),v=p?d.find("audio"):d.find("embed"),v=v.get(0);if(p){d.find("audio").css({width:0,height:0,visibility:"hidden"});d.append('<div class="'+n.time+" "+n.timeCurrent+'"></div><div class="'+n.bar+'"><div class="'+n.barLoaded+'"></div><div class="'+n.barPlayed+'"></div></div><div class="'+n.time+" "+n.timeDuration+'"></div><div class="'+n.volume+'"><div class="'+n.volumeButton+'" title="'+t.strVolume+'"><a href="#">'+t.strVolume+'</a></div><div class="'+n.volumeAdjust+'"><div><div></div></div></div></div>');var m=d.find("."+n.bar),g=d.find("."+n.barPlayed),y=d.find("."+n.barLoaded),b=d.find("."+n.timeCurrent),w=d.find("."+n.timeDuration),E=d.find("."+n.volumeButton),S=d.find("."+n.volumeAdjust+" > div"),x=0,T=function(e){theRealEvent=i?e.originalEvent.touches[0]:e;v.currentTime=Math.round(v.duration*(theRealEvent.pageX-m.offset().left)/m.width())},N=function(e){theRealEvent=i?e.originalEvent.touches[0]:e;v.volume=Math.abs((theRealEvent.pageY-(S.offset().top+S.height()))/S.height())},C=function(){var e=setInterval(function(){if(v.buffered.length<1)return true;y.width(v.buffered.end(0)/v.duration*100+"%");if(Math.floor(v.buffered.end(0))>=Math.floor(v.duration))clearInterval(e)},100)};var k=v.volume,L=v.volume=.111;if(Math.round(v.volume*1e3)/1e3==L)v.volume=k;else d.addClass(n.noVolume);w.html("…");b.html(f(0));v.addEventListener("loadeddata",function(){C();w.html(e.isNumeric(v.duration)?f(v.duration):"…");S.find("div").height(v.volume*100+"%");x=v.volume});v.addEventListener("timeupdate",function(){b.html(f(v.currentTime));g.width(v.currentTime/v.duration*100+"%")});v.addEventListener("volumechange",function(){S.find("div").height(v.volume*100+"%");if(v.volume>0&&d.hasClass(n.muted))d.removeClass(n.muted);if(v.volume<=0&&!d.hasClass(n.muted))d.addClass(n.muted)});v.addEventListener("ended",function(){d.removeClass(n.playing).addClass(n.stopped)});m.on(s,function(e){T(e);m.on(o,function(e){T(e)})}).on(a,function(){m.unbind(o)});E.on("click",function(){if(d.hasClass(n.muted)){d.removeClass(n.muted);v.volume=x}else{d.addClass(n.muted);x=v.volume;v.volume=0}return false});S.on(s,function(e){N(e);S.on(o,function(e){N(e)})}).on(a,function(){S.unbind(o)})}else d.addClass(n.mini);d.addClass(c?n.playing:n.stopped);d.find("."+n.playPause).on("click",function(){if(d.hasClass(n.playing)){e(this).attr("title",t.strPlay).find("a").html(t.strPlay);d.removeClass(n.playing).addClass(n.stopped);p?v.pause():v.Stop()}else{e(this).attr("title",t.strPause).find("a").html(t.strPause);d.addClass(n.playing).removeClass(n.stopped);p?v.play():v.Play()}return false});r.replaceWith(d)});return this}})(jQuery,window,document);$(document).ready(function(){$("#podcast_submit").click(function(){title=encodeURIComponent($("#podcast_title").val());post=encodeURIComponent($("#podcast_post").val());file=encodeURIComponent($("#podcast_file").val());$.ajax({type:"POST",url:"../../../publish_podcast.php",data:"title="+title+"&post="+post+"&file="+file,success:function(html)
{if(html=='true')
{window.location.reload();}
else
{$('#newPodcast').modal('hide');$("#top_msg").css('display','inline','important');$("#top_msg").html('<div class="alert alert-danger alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>'+html+'</div>');}}});return false;});$('#newPodcast').on('hide.bs.modal',function(e){podFile=encodeURIComponent($("#podcast_file").val());if(podFile!='')
{$.ajax({type:"POST",url:"../../../delete_podcast_file.php",data:"file="+podFile,success:function(html)
{if(html)
{$("#uploadedPodcasts").html('');$("#podcast_file").val('');}}});}});$('#editPodcast').on('show.bs.modal',function(e){$("#edit_podcast_post").val("");$("#edit_podcast_title").val("");$("#edit_podcast_file").val("");userID=encodeURIComponent($(e.relatedTarget).attr("id"));$("#edit_podcast_postid").val(userID);$.ajax({type:"POST",url:"../../../get_title_content.php",data:"id="+userID,success:function(html)
{if(html)
{$("#edit_podcast_title").val(html);}}});$.ajax({type:"POST",url:"../../../get_post_content.php",data:"id="+userID,success:function(html)
{if(html)
{$("#edit_podcast_post").val(html);}}});$.ajax({type:"POST",url:"../../../get_podcast_file.php",data:"id="+userID,success:function(html)
{if(html)
{var fileList=html.split(',');for(var i=0;i<fileList.length;i++)
{$("#edit_uploadedPodcasts").append('<div class="alert alert-success uploaded_file_'+fileList[i].replace(".","_")+'"><button type="button" class="close podcast_file_delete" id="'+fileList[i]+'">&times;</button>'+fileList[i]+'</div>');linkEditFileDelete('.podcast_file_delete');if($("#edit_podcast_file").val()=='')
{$("#edit_podcast_file").val(fileList[i]);}
else
{$("#edit_podcast_file").val($("#edit_podcast_file").val()+','+fileList[i]);}}}}});});$('#editPodcast').on('hide.bs.modal',function(e){userID=encodeURIComponent($("#edit_podcast_postid").val());$.ajax({type:"POST",url:"../../../get_podcast_file.php",data:"id="+userID,success:function(html)
{if(html)
{podFile=encodeURIComponent($("#edit_podcast_file").val());var fileList=$("#edit_podcast_file").val().split(',');var oldFileList=html.split(',');for(var i=0;i<fileList.length;i++)
{var index=oldFileList.indexOf(fileList[i]);if(index==-1)
{$.ajax({type:"POST",url:"../../../delete_podcast_file.php",data:"file="+fileList[i],success:function(response)
{if(response)
{$("#edit_uploadedPodcasts").html('');$("#edit_podcast_file").val('');}}});}}}}});});$("#edit_submit").click(function(){postID=encodeURIComponent($("#edit_podcast_postid").val());title=encodeURIComponent($("#edit_podcast_title").val());post=encodeURIComponent($("#edit_podcast_post").val());file=encodeURIComponent($("#edit_podcast_file").val());$.ajax({type:"POST",url:"../../../edit_podcast.php",data:"postID="+postID+"&title="+title+"&post="+post+"&file="+file,success:function(html)
{if(html=='true')
{window.location.reload();}
else
{$('#editPodcast').modal('hide');$("#top_msg").css('display','inline','important');$("#top_msg").html('<div class="alert alert-danger alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>'+html+'</div>');}}});return false;});$("#comment_submit").click(function(){$('#newComment').modal('hide');postID=encodeURIComponent($("#post_id").val());post=encodeURIComponent($("#comment_post").val());$.ajax({type:"POST",url:"../../../includes/add_comment.php",data:"postID="+postID+"&service=podcast&comment="+post,success:function(html)
{if(html=='true')
{window.location.reload();}
else
{$("#top_msg").css('display','inline','important');$("#top_msg").html('<div class="alert alert-danger alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>'+html+'</div>');}}});return false;});$('#editComment').on('show.bs.modal',function(e){$("#edit_comment_post").val("");commentID=encodeURIComponent($(e.relatedTarget).attr("id"));$("#edit_comment_postid").val(commentID);$.ajax({type:"POST",url:"../../../includes/get_comment_content.php",data:"id="+commentID,success:function(html)
{if(html)
{$("#edit_comment_post").val(html);}}});});$("#edit_comment_submit").click(function(){$('#editComment').modal('hide');postID=encodeURIComponent($("#edit_comment_postid").val());post=encodeURIComponent($("#edit_comment_post").val());$.ajax({type:"POST",url:"../../../includes/edit_comment.php",data:"commentID="+postID+"&post="+post,success:function(html)
{if(html=='true')
{window.location.reload();}
else
{$("#top_msg").css('display','inline','important');$("#top_msg").html('<div class="alert alert-danger alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>'+html+'</div>');}}});return false;});});$(function(){$('#uploadPodcast').fileupload({dataType:'json',url:'../../../upload_podcast.php',paramName:'file',done:function(e,data){$("#uploadedPodcasts").html('');if(!data.result.error)
{podFile=encodeURIComponent($("#podcast_file").val());if(podFile!='')
{$("#podcast_file").val(podFile+','+data.result.file.name);}
else
{$("#podcast_file").val(data.result.file.name);}
$("#uploadedPodcasts").append('<div class="alert alert-success uploaded_file_'+data.result.file.name.replace(".","_")+'"><button type="button" class="close podcast_file_delete" id="'+data.result.file.name+'">&times;</button>'+data.result.file.name+'</div>');linkFileDelete('.podcast_file_delete');}
else
{$("#top_msg").css('display','inline','important');$("#top_msg").html('<div class="alert alert-danger alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>'+data.result.error.message+'</div>');}}});$('#edit_uploadPodcast').fileupload({dataType:'json',url:'../../../upload_podcast.php',paramName:'file',done:function(e,data){if(!data.result.error)
{podFile=encodeURIComponent($("#edit_podcast_file").val());if(podFile!='')
{$("#edit_podcast_file").val(podFile+','+data.result.file.name);}
else
{$("#edit_podcast_file").val(data.result.file.name);}
$("#edit_uploadedPodcasts").append('<div class="alert alert-success uploaded_file_'+data.result.file.name.replace(".","_")+'"><button type="button" class="close podcast_file_delete" id="'+data.result.file.name+'">&times;</button>'+data.result.file.name+'</div>');linkEditFileDelete('.podcast_file_delete');}
else
{$("#top_msg").css('display','inline','important');$("#top_msg").html('<div class="alert alert-danger alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>'+data.result.error.message+'</div>');}}});});function loadMorePosts(start,count)
{blog_id=encodeURIComponent($(".podcast-main").attr("id"));$.ajax({type:"POST",url:"../../../get_post.php",data:"userID="+blog_id+"&postCount="+count+"&startPost="+start,success:function(html)
{if(html)
{$(".podcast-main").append(html);linkPostDelete('.delete_post');linkAudioPlayer('audio');$(window).bind('scroll',bindScrollPosts);}}});}
function loadMoreComments(start,count)
{post_id=encodeURIComponent($(".post-comments").attr("id"));$.ajax({type:"POST",url:"../../../includes/get_comment.php",data:"postID="+post_id+"&service=podcast&postCount="+count+"&startPost="+start,success:function(html)
{if(html)
{$(".post-comments").append(html);linkCommentDelete('.delete_comment');$(window).bind('scroll',bindScrollComments);}}});}
function bindScrollPosts()
{if($(window).scrollTop()+$(window).height()>$(document).height()-100)
{$(window).unbind('scroll');loadMorePosts(start_post,posts);start_post=start_post+posts;}}
function bindScrollComments()
{if($(window).scrollTop()+$(window).height()>$(document).height()-100)
{$(window).unbind('scroll');loadMoreComments(start_post,posts);start_post=start_post+posts;}}
function linkPostDelete(selector)
{$(selector).click(function(){var object=$(this);post_id=encodeURIComponent(object.attr("id"));bootbox.confirm("Are you sure you want to delete your post?",function(result){if(result)
{$.ajax({type:"POST",url:"../../../delete_podcast.php",data:"id="+post_id,success:function(html)
{if(html=='true')
{window.location.reload();}
else
...
{$(selector).click(function(){var object=$(this);post_id=encodeURIComponent(object.attr("id"));bootbox.confirm("Are you sure you want to delete your comment?",function(result){if(result)
{$.ajax({type:"POST",url:"../../../includes/delete_comment.php",data:"id="+post_id,success:function(html)
{if(html=='true')
{window.location.reload();}
else
{$("#top_msg").css('display','inline','important');$("#top_msg").html('<div class="alert alert-danger alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>'+html+'</div>');}}});}});});}
function linkFileDelete(selector)
{$(selector).click(function(){var object=$(this);podFile=encodeURIComponent(object.attr('id'));allFiles=$("#podcast_file").val();$.ajax({type:"POST",url:"../../../delete_podcast_file.php",data:"file="+podFile,success:function(html)
{var fileList=allFiles.split(',');var index=fileList.indexOf(podFile);if(index!=-1)
{fileList.splice(index,1);$("#podcast_file").val(fileList.toString());}
$(".uploaded_file_"+podFile.replace(".","_")).remove();}});return false;});}
function linkEditFileDelete(selector)
{$(selector).click(function(){var object=$(this);podFile=encodeURIComponent(object.attr('id'));allFiles=$("#edit_podcast_file").val();$.ajax({type:"POST",url:"../../../delete_podcast_file.php",data:"file="+podFile,success:function(html)
{var fileList=allFiles.split(',');var index=fileList.indexOf(podFile);if(index!=-1)
{fileList.splice(index,1);$("#edit_podcast_file").val(fileList.toString());}
$(".uploaded_file_"+podFile.replace(".","_")).remove();}});return false;});}
function linkAudioPlayer(selector)
{$(selector).audioPlayer({classPrefix:'audioplayer'});}