...
else{$.each(obj,function(key,value){option(key,value);});}},submit:function(){this.onSubmit();$.each(options.params,function(key,value){form.append($('<input '+'type="hidden" '+'name="'+key+'" '+'value="'+value+'" '+'/>'));});form.submit();iframe.unbind().load(function(){var myFrame=document.getElementById(iframe.attr('name'));var response=$(myFrame.contentWindow.document.body).text();self.onComplete(response);});}});}})(jQuery);$(document).ready(function(){$("#blog_submit").click(function(){$('#newPost').modal('hide');userID=encodeURIComponent($("#blog_userid").val());title=encodeURIComponent($("#blog_title").val());post=encodeURIComponent($("#blog_post").val());$.ajax({type:"POST",url:"../../../publish_blog.php",data:"userID="+userID+"&title="+title+"&post="+post,success:function(html)
{if(html=='true')
{window.location.reload();}
else
{$("#top_msg").css('display','inline','important');$("#top_msg").html('<div class="alert alert-danger alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>'+html+'</div>');}}});return false;});$('#editPost').on('show.bs.modal',function(e){$("#edit_blog_post").val("");userID=encodeURIComponent($(e.relatedTarget).attr("id"));$("#edit_blog_postid").val(userID);$.ajax({type:"POST",url:"../../../get_title_content.php",data:"id="+userID,success:function(html)
{if(html)
{$("#edit_blog_title").val(html);}}});$.ajax({type:"POST",url:"../../../get_post_content.php",data:"id="+userID,success:function(html)
{if(html)
{$("#edit_blog_post").val(html);}}});});$("#edit_submit").click(function(){$('#editPost').modal('hide');postID=encodeURIComponent($("#edit_blog_postid").val());userID=encodeURIComponent($("#edit_blog_userid").val());title=encodeURIComponent($("#edit_blog_title").val());post=encodeURIComponent($("#edit_blog_post").val());$.ajax({type:"POST",url:"../../../edit_blog.php",data:"userID="+userID+"&postID="+postID+"&title="+title+"&post="+post,success:function(html)
{if(html=='true')
{window.location.reload();}
else
{$("#top_msg").css('display','inline','important');$("#top_msg").html('<div class="alert alert-danger alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>'+html+'</div>');}}});return false;});$("#comment_submit").click(function(){$('#newComment').modal('hide');postID=encodeURIComponent($("#post_id").val());post=encodeURIComponent($("#comment_post").val());$.ajax({type:"POST",url:"../../../includes/add_comment.php",data:"postID="+postID+"&service=blog&comment="+post,success:function(html)
{if(html=='true')
{window.location.reload();}
else
...
{$("#top_msg").css('display','inline','important');$("#top_msg").html('<div class="alert alert-danger alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>'+html+'</div>');}}});return false;});var imageUpload=$('#upload_image').upload({name:'file',action:'../../../includes/upload.php',enctype:'multipart/form-data',params:{},autoSubmit:true,onSubmit:function(){$("#image_url").val('Uploading Image...');},onComplete:function(filename){obj=JSON.parse(filename);if(!obj.error)
{$("#image_url").val(obj.results.file.name);}
else
{$("#image_url").val('Error Uploading');}}});});function loadMorePosts(start,count)
{blog_id=encodeURIComponent($(".blog-main").attr("id"));$.ajax({type:"POST",url:"../../../get_post.php",data:"userID="+blog_id+"&postCount="+count+"&startPost="+start,success:function(html)
{if(html)
{$(".blog-main").append(html);linkPostDelete('.delete_post');$(window).bind('scroll',bindScrollPosts);}}});}
function loadMoreComments(start,count)
{post_id=encodeURIComponent($(".post-comments").attr("id"));$.ajax({type:"POST",url:"../../../includes/get_comment.php",data:"postID="+post_id+"&service=blog&postCount="+count+"&startPost="+start,success:function(html)
{if(html)
...
{$.ajax({type:"POST",url:"../../../delete_blog.php",data:"id="+post_id,success:function(html)
{if(html=='true')
{window.location.reload();}
else
